<div>
    <div class="float-left" style="padding-top: 25px; padding-right: 30px;">
        {{count($data)}} of {{$totalItems}} items
    </div>
    <div class="float-right"><br>
        <ul class="pager" forForm="{{$forForm}}">
            ...
            ?>
            @if (1 != $page)
                <li><a class="link-submit" href="#" action="{{ URL($url . 1) }}">First</a></li>
            @endif
            ...
            @if ($limitPage > 0)
                @for ($i; $i <= $limitPage; $i++)
                    <li>
                        ...
                        @endif
                        <a class="link-submit{{$class}}" href="#" action="{{ URL($url . $i) }}">
            ...
            @if ($totalPage != $page)
                <li><a class="link-submit" href="#" action="{{ URL($url . $totalPage) }}">{{$totalPage}} - End</a></li>
            @endif
        </ul>
    </div>
